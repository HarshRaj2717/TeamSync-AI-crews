...
async def extract_code(input_str):
...
async def execute_code(extracted_code):
...
async def generate_code_output(prompt):
    result = llm.invoke(prompt)
    return await extract_code(result.content)

async def ask_grok(user_prompt: str):
    prompt = f"""
    ...
    while True:
        ...
        code = await generate_code_output(prompt)
        print("Extracted Code:", code)

        exe = await execute_code(code)
    ...
    final_code = await generate_code_output(prompt)
    final_exe = await execute_code(final_code)
